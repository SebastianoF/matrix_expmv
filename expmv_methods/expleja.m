function [phikAv, errest, info, extreigs] = expleja (h, A, varargin)
...
if (isfloat (A))
    ...
    if (nargin >= 4)
        tol = varargin{2};
        if (length (tol) == 1), tol(2) = 0; tol(3) = Inf;
        elseif (length (tol) == 2), tol(3) = Inf; end
        %default value
    else 
        tol = [0,2^(-24),Inf];
    end
    % get spectral estimate
    if (nargin == 5), extreigs = varargin{3}; else extreigs = gersh(A); end
else
...
if (abs(SR)+abs(LR)+abs(LI2) == 0)
    phikAv = v;     errest = 0;    info = 0;    return
end
...
phikAv = v; errest = zeros(nsteps,1); info = zeros(nsteps,1);
for j = 1:nsteps
    [phikAv,perrest,pinfo] = newt(h,A,phikAv,xi,dd,gammafoc,...
...
if isempty(nsteps) || (nsteps==maxnsteps && max(stepperit)>maxstepperit(tmp))
    error('reached substep limit of %d', maxsteps);
end
...
%
function [y, normerrest, m] = newton_fix(h, A, v, xi, dd, gammafoc,...
...
% but the desired error is not reached INFO contains -MAX_NUMBER_OF_STEPS.
%
function [y, normerrest, m] = newtons_fix (h, A, v, xi, dd, gammafoc, ...
    abstol, reltol, nnorm, iter)
%H=diag(h);
%iter=iter;
maxm = size (dd, 1);
...
% 1î*[-2,2]. Only the first 200 Leja points are availabel --> M<=200.
%
function xi = lejas(m)
% symmetric
